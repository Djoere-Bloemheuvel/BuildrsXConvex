...
// OPTIMIZED VERSION: Get deals by pipeline with batch company lookups
export const getByPipelineOptimized = query({
  args: { pipelineId: v.id("pipelines") },
  returns: v.array(v.object({
    _id: v.id("deals"),
    _creationTime: v.number(),
  ...
  handler: async (ctx, args) => {
    console.log(`🎯 Deals Optimized: Getting deals for pipeline ${args.pipelineId}`);
    
    // Step 1: Get all deals for pipeline (same as original)
    const deals = await ctx.db
      .query("deals")
      .withIndex("by_pipeline", (q) => q.eq("pipelineId", args.pipelineId))
      .collect();
    ...
    // Step 2: OPTIMIZATION - Batch company lookups instead of N+1 queries
    ...
    console.log(`🏢 Fetching ${uniqueCompanyIds.length} unique companies in batch`);
    ...
    if (uniqueCompanyIds.length > 0) {
      const companiesData = await Promise.all(
        uniqueCompanyIds.map(async (companyId) => {
          try {
            ...
            return { id: companyId, company: null };
          }
        })
      );
      ...
      for (const { id, company } of companiesData) {
        if (company) {
          companiesMap.set(id, { name: company.name });
        }
      }
    }
    
    // Step 3: Enrich deals with company data (no additional queries!)
    const enrichedDeals = deals.map(deal => {
      const companies = deal.companyId ? companiesMap.get(deal.companyId) : undefined;
    ...
    console.log(`✅ Optimized deals query completed - ${deals.length} deals with ${uniqueCompanyIds.length} companies fetched in batch`);
